...
import numpy as np
import pickle
from holly_data.holly_util import Holly_util
...
class HollywoodSub(chainer.dataset.DatasetMixin):
	"""
	Args:
...
class Hollywood(chainer.dataset.DatasetMixin):
	NUM_OF_CLASSES = 12
	LABELS = ['HandShake', 'DriveCar', 'FightPerson', 'Eat', 'StandUp', 'SitUp', 'Kiss',
	...
	def __init__(self, path, mean=None, frames=6, data_aug=True):
		...
		self.data_dic = Holly_util().get_data_dic()
	
	def refactor(self, li):
		# new_l = [0 for x in range(len(li))]
	...
	def get_label_idx(self, path):
		...
		return self.refactor(self.data_dic[img_dir])
	...
	def get_example(self, i):
		...
		base = HollywoodSub(image_dir)
		w, h = base.size()
		crop_size = int(w * 0.7)
		if self.data_aug:
			mask_area, mask_value = Hollywood.random_erasing(crop_size, crop_size)
			base.init(
				crop_size=crop_size,
		...
		return images, self.get_label_idx(image_dir)
